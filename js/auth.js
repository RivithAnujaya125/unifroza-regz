...
export async function registerUser(username, email, password) {
  try {
    ...
    // Create a document in Firestore to track their Elite status
...
/**
 * Check if a user is an Elite member by querying Firestore
 */
export async function checkEliteStatus(uid) {
  try {
    const docRef = doc(db, "users", uid);
    const docSnap = await getDoc(docRef);

    if (docSnap.exists()) {
      return docSnap.data().isElite === true;
    } else {
